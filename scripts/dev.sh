...
#
# ASCII only on purpose: this runs under macOS system bash 3.2, which mangles a
# non-braced $VAR followed by a multibyte char in a UTF-8 locale.
#
# Usage:
...
wait_for() { # wait_for <fn> <label>
  local i
  for i in $(seq 1 30); do "${1}" && return 0; sleep 1; done
  echo "[x] ${2} did not come up in time"; return 1
}

start_pg() {
  if pg_up; then echo "[ok] postgres already running"; return; fi
...
start_api() {
  if api_up; then echo "[ok] api already running on :${API_PORT}"; return; fi
  ...
  echo "[ok] api up  (logs: .dev/api.log)"
}

start_web() {
  if web_up; then echo "[ok] web already running on :${WEB_PORT}"; return; fi
  ...
  echo "[ok] web up  (logs: .dev/web.log)"
}
